...
压缩解压 普通模式经exec调用远端zip/tar/unzip 支持zip与tar.gz/tgz 路径逐项shell转义并用临时包完成后替换目标 tar条目不带./前缀以兼容Windows解压查看器 避免失败破坏已有压缩包 解压会覆盖同名内容需前端确认 工具缺失明确报错 一次性exec不继承sudo SFTP提权故sudo模式禁止
