...
操作 list_dir列举目录返回FileEntry列表 目录在前文件在后按名排序 read_file读取 write_file覆盖写 remove_file删文件 remove_dir删空目录 create_dir建目录 rename重命名移动 canonicalize解析绝对路径用于定位主目录 upload本地文件读入再写远端 download远端读入再写本地
