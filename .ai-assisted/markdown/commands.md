...
命令分类 ssh_connect/disconnect terminal_open/write/resize path_is_dir(本地路径是否目录 供终端拖拽上传前拒绝文件夹) monitor_collect sftp_* transfer_* 详细签名见api.ts(前端)与commands.rs(后端)
