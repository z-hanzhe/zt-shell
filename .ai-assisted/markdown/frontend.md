...
组件 src/components Icon.vue内置SVG图标避免图标库依赖 AppDialog.vue通用提示/确认/输入弹窗 MonitorPanel.vue左监控 TerminalPanel.vue右上终端区含选项卡栏文件夹图标开连接管理器与设置按钮 Terminal.vue封装xterm单终端 BottomPanel.vue右下含选项卡默认文件管理器 FileManager.vue文件管理器 ConnectionManager.vue连接管理器弹窗参考conn.png ConnectionEditor.vue连接编辑弹窗 SettingsDialog.vue设置弹窗 TitleBar.vue自绘标题栏
...
文件管理 FileManager.vue左侧为根目录/起始文件树 根默认展开且不显示展开图标 单击树节点右侧显示目录 双击或箭头展开收起 右侧目录变化后树选中项自动滚入可视区域 左右分割线可拖拽 地址栏右侧↑将地址栏路径cd到当前终端 ↓通过终端printf专用OSC标记读取PWD后切换文件管理路径 新建目录弹窗提示完整目标路径 右侧表头点击排序默认文件名升序 升序文件夹在上降序文件夹在下 非根目录...始终置顶 行内操作按钮row-ops用visibility切换非display且tbody td固定height26px 避免悬停出现按钮致行变高抖动
